...
class Database:
    ...
    async def add_user(self, telegram_id, name, sure_name, age, phone):
        query = """
        INSERT INTO users(telegram_id, name, sure_name, age, phone)
        VALUES ($1, $2, $3, $4, $5)
        """
        await self.pool.execute(query,telegram_id,name,sure_name, int(age),phone)
    ...
    async def user_profile(self, telegram_id):
        query = """
        select name,sure_name, age, phone, role from users where telegram_id=$1;
        """
    ...
    async def add_products(self,name,price,description):
        query="""
    ...
    async def get_order_history(self, user_id):
        ...
        orders = await self.pool.fetch(
            """
            SELECT o.created_at, p.name, p.price, p.id
            FROM order_items oi
            JOIN orders o ON oi.order_id=o.id
            JOIN products p ON oi.product_id=p.id
    ...
    async def confirm_order(self, user_id):
        ...
        confirm = await self.pool.execute(
            """
            update orders set order_status='ordered' where user_id=$1
            """,
            user_id
        )
